...
И помните: правда редко лежит на поверхности.`,
...

  {
    ...
    id: 2, folder: 'inbox', tab: 'primary', starred: true, read: false,
    from: { name: 'Лаб. Форензик', email: 'lab@ashford-forensics.gov', avatar: '🧪' },
    subject: 'СРОЧНО: Результаты анализа почвы — Дело #001',
    preview: 'Обнаружены следы вещества NC-7 — нейротоксина класса III. Производство: исключительно NovaChem...',
    date: '07:41',
    body: `Детектив,

Отчёт по образцам почвы с восточной парковки NovaChem готов.

КРАТКОЕ РЕЗЮМЕ:
  Обнаружены следы вещества NC-7 — нейротоксина
  класса III. Производство: исключительно NovaChem.

  Вещество разрушается за 72 часа. Это значит,
  что у нас очень мало времени.

  Следы волочения ведут к канализационному люку.

⚠ NovaChem подала запрос на засекречивание отчёта.
  Я отклонил. Будьте готовы к давлению.

Ст. аналитик Д. Крейн`,
    attachments: [{ name: 'forensic_report_FL-2024-0441.pdf', size: '1.1 МБ', icon: '📄' }],
  },
  {
    id: 3, folder: 'inbox', tab: 'primary', starred: false, read: true,
    from: { name: 'NovaChem Legal', email: 'legal@novachem-corp.com', avatar: '⚖️' },
    subject: 'Официальное уведомление — прекратить расследование',
    preview: 'Настоящим уведомляем вас, что любые дальнейшие следственные действия в отношении сотрудников...',
    date: 'Вчера',
    body: `Уважаемый детектив,

Настоящим уведомляем вас, что любые дальнейшие
следственные действия в отношении сотрудников,
объектов и документации NovaChem Corp должны быть
НЕМЕДЛЕННО ПРЕКРАЩЕНЫ.

Нарушение повлечёт иск на $14 млн и уголовное
преследование за промышленный шпионаж.

Мы знаем, где находится ваш офис.

юрист Харрис Блейк, LLM`,
    attachments: [],
  },
  {
    id: 4, folder: 'inbox', tab: 'primary', starred: false, read: false,
    from: { name: '??? ???', email: 'noreply@████.onion', avatar: '👤' },
    subject: 'Она ещё жива. Но не долго.',
    preview: 'Вы смотрите не туда. Настоящий заказчик — выше. Намного выше. Проверьте 14-й кабинет...',
    date: 'Вчера',
    body: `Детектив,

Вы смотрите не туда.

Вас интересует Вэйн — но он лишь инструмент.
Настоящий заказчик — выше. Намного выше.

Проверьте 14-й кабинет. Исполнительный этаж.
Там была встреча в ту ночь.

У вас есть 48 часов.
После — Елену не найдут.
...
export default function MailApp({ onNotificationRead }) {
  ...
  const folderMails = (() => {
    ...
    if (folder === 'trash')   return mails.filter(m => m.folder === 'trash')
    if (folder === 'sent')    return mails.filter(m => m.folder === 'sent')
    if (folder === 'drafts')  return mails.filter(m => m.folder === 'drafts')
    return mails.filter(m => m.folder === 'inbox' && m.tab === tab)
  })()
